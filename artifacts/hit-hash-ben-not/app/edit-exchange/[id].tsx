...
export default function EditExchangeScreen() {
  ...
  const [spentCurrency, setSpentCurrency] = useState<(typeof EXCHANGE_CURRENCIES)[number]>(
    (exchange?.spentCurrency as (typeof EXCHANGE_CURRENCIES)[number]) ?? "USD"
  );
  const [receivedCurrency, setReceivedCurrency] = useState<(typeof EXCHANGE_CURRENCIES)[number]>(
    (exchange?.receivedCurrency as (typeof EXCHANGE_CURRENCIES)[number]) ?? "EUR"
  );
  ...
  async function handleSave() {
    if (!exchange || !amountSpent || !amountReceived || !date) return;
    setSaving(true);
    try {
  ...
  const canSave = !!amountSpent && !!amountReceived && !!date;
