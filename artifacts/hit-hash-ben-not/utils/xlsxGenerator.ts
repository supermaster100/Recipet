...
export function buildXLSXBase64(
  ...
  const wsTravels = XLSX.utils.aoa_to_sheet([
    ...
    ...travels.map((t) => [t.num, t.departureDate, t.departureCountry, t.departureCity,
      ...
      t.description, photoName(t.photo)]),
  ]);
  ...
  const wsExchanges = XLSX.utils.aoa_to_sheet([
    ...
    ...exchanges.map((x) => [x.date, x.amountSpent, x.spentCurrency, x.amountReceived, x.receivedCurrency, x.note, photoName(x.photo)]),
  ]);
  ...
  const wsATM = XLSX.utils.aoa_to_sheet([
    ...
    ...atmWithdrawals.map((a) => [a.date, a.cardLastFour, a.amount, a.currency, photoName(a.photo)]),
  ]);
  ...
  const wsMoneyTransfers = XLSX.utils.aoa_to_sheet([
    ...
    ...moneyTransfers.map((m) => [m.receiptName, m.giverName, m.workerNumber, m.date, m.amount, m.currency, photoName(m.photo)]),
  ]);
  ...
  const wsClientTransfers = XLSX.utils.aoa_to_sheet([
    ...
    ...clientTransfers.map((c) => [c.clientName, c.giverName, c.date, c.amount, c.currency, photoName(c.photo)]),
  ]);
